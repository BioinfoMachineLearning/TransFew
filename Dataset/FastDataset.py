...
class FastTransFunDataset(Dataset):
...
'''class TestDataset(Dataset):
    def __init__(self, data_pth=None, term_indicies=None, submodel=None):

        self.submodel = submodel

        data = pickle_load(data_pth)

        labels = pickle_load("tst_labesl")['cc']
        ...
        self.interpro_features = data['interpro']
        self.labels = labels
        self.term_indicies = term_indicies
    ...
    def __getitem__(self, index):
        ...
        if self.submodel == 'esm2_t48':
            lab = torch.tensor(self.labels[proteins], dtype=torch.float32).view(1, -1)
            lab = torch.index_select(lab, 1, self.term_indicies)
            return esm, lab #proteins
        elif self.submodel == 'msa_1b':
            return msa, proteins
        elif self.submodel == 'diamond':
